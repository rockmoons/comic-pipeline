...
from utils.parser import (
    ...
    extract_story_word_count, extract_forbidden_words,
    extract_style_token_presence,
)
...
def check_chapter_title_format(story_text: str) -> CheckResult:
    ...
    import re
    if re.search(r'第\d+章[：:]\s*[^\s]{2,8}', story_text):
...
def check_transition_count(director_text: str) -> CheckResult:
    ...
    count = director_text.count('transition')
    if count <= 2:
...
def check_shot_variety(director_text: str) -> CheckResult:
    ...
    import re
    shot_types = re.findall(r'景别[：:]\s*(\S+)', director_text)
